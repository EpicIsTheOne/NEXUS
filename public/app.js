...
const el = {
  ...
  mobileBlurReset: document.getElementById('mobile-blur-reset'),
  newChat: document.getElementById('new-chat'),
  openLibrary: document.getElementById('open-library'),
  profileButton: document.getElementById('profile-button'),
  imageGenerateInlineBtn: document.getElementById('image-generate-inline-btn'),
  imageGenerateInlinePanel: document.getElementById('image-generate-inline-panel'),
  closeImageGenerateInline: document.getElementById('close-image-generate-inline'),
  imageGenerateInlinePrompt: document.getElementById('image-generate-inline-prompt'),
  imageGenerateInlinePromptFromChat: document.getElementById('image-generate-inline-prompt-from-chat'),
  imageGenerateInlineModel: document.getElementById('image-generate-inline-model'),
  imageGenerateInlineUseAvatar: document.getElementById('image-generate-inline-use-avatar'),
  imageGenerateInlineReferenceInput: document.getElementById('image-generate-inline-reference-input'),
  imageGenerateInlineReferenceList: document.getElementById('image-generate-inline-reference-list'),
  imageGenerateInlineStyle: document.getElementById('image-generate-inline-style'),
  imageGenerateInlineAspect: document.getElementById('image-generate-inline-aspect'),
  imageGenerateInlineNegative: document.getElementById('image-generate-inline-negative'),
  imageGenerateInlineVariation: document.getElementById('image-generate-inline-variation'),
  imageGenerateInlineRetry: document.getElementById('image-generate-inline-retry'),
  imageGenerateInlineSubmit: document.getElementById('image-generate-inline-submit'),
  imageGenerateInlineStatus: document.getElementById('image-generate-inline-status'),
  ...
  personaEditDialog: document.getElementById('persona-edit-dialog'),
  personaEditTitle: document.getElementById('persona-edit-title'),
  closePersonaEdit: document.getElementById('close-persona-edit'),
  personaEditForm: document.getElementById('persona-edit-form'),
  personaEditName: document.getElementById('persona-edit-name'),
  personaEditDescription: document.getElementById('persona-edit-description'),
  personaEditDefault: document.getElementById('persona-edit-default'),
  personaEditCancel: document.getElementById('persona-edit-cancel'),
  ...
  createCharacterType: document.getElementById('create-character-type'),
  ...
  assistantConfigFields: document.getElementById('assistant-config-fields'),
  assistantOpenclawAgent: document.getElementById('assistant-openclaw-agent'),
  assistantRuntime: document.getElementById('assistant-runtime'),
  assistantPermissionTier: document.getElementById('assistant-permission-tier'),
  assistantBadgeLabel: document.getElementById('assistant-badge-label'),
  assistantThinking: document.getElementById('assistant-thinking'),
  assistantPersonalityPrompt: document.getElementById('assistant-personality-prompt'),
  assistantStyleNotes: document.getElementById('assistant-style-notes'),
  ...
  imageGenerateDialog: document.getElementById('image-generate-dialog'),
  closeImageGenerate: document.getElementById('close-image-generate'),
  imageGenerateForm: document.getElementById('image-generate-form'),
  imageGeneratePrompt: document.getElementById('image-generate-prompt'),
  imageGenerateStyle: document.getElementById('image-generate-style'),
  imageGenerateAspect: document.getElementById('image-generate-aspect'),
  imageGenerateNegative: document.getElementById('image-generate-negative'),
  imageGenerateCancel: document.getElementById('image-generate-cancel'),
  imageGenerateVariation: document.getElementById('image-generate-variation'),
  imageGenerateRetry: document.getElementById('image-generate-retry'),
  imageGenerateSubmit: document.getElementById('image-generate-submit'),
  imageGenerateStatus: document.getElementById('image-generate-status'),
...
function getCharacterType(character = getSelectedCharacter()) { return String(character?.type || 'character') === 'assistant' ? 'assistant' : 'character'; }
function isAssistantCharacter(character = getSelectedCharacter()) { return getCharacterType(character) === 'assistant'; }
function getAssistantMetaFromCharacter(character = getSelectedCharacter()) {
  if (!character || !isAssistantCharacter(character)) return null;
  const config = character.assistantConfig || {};
  return {
    type: 'assistant',
    badgeLabel: config.badgeLabel || 'OpenClaw',
    runtime: config.runtime || 'subagent',
    permissionTier: config.permissionTier || 'chat',
  };
}
function getCurrentConversationAssistantMeta() {
  const conversation = getSelectedConversation();
  if (conversation?.assistantMeta) return conversation.assistantMeta;
  return getAssistantMetaFromCharacter();
}
function getAssistantBadgeLabel() {
  return getCurrentConversationAssistantMeta()?.badgeLabel || 'OpenClaw';
}
...
function getCharacterTtsReadNarration() { return state.selectedCharacterSettings?.ttsReadNarration === true; }
...
function getMemoryEls() {
  return {
    ...
    addPinned: document.getElementById('memory-add-pinned'),
    refresh: document.getElementById('memory-refresh'),
...
async function loadMemoryBundle() {
  ...
  if (!state.selectedConversationId || !getSelectedConversation()) {
    await ensureConversation().catch(() => null);
  }
  if (!state.selectedConversationId || !getSelectedConversation()) {
    state.memoryBundle = null;
    renderMemoryPanel();
  ...
  try {
    ...
    state.memoryBundle = null;
    state.memoryError = String(error?.message || error || 'Memory failed to load');
    renderMemoryPanel();
    throw error;
  }
  renderMemoryPanel();
...
async function saveManualPinnedMemory() {
  if (!state.selectedCharacterId) return;
  if (!state.selectedConversationId) await ensureConversation().catch(() => null);
  if (!state.selectedConversationId) return;
  const text = prompt('Add pinned memory:', '');
  if (!text || !text.trim()) return;
  await api(`./api/conversations/${encodeURIComponent(state.selectedConversationId)}/memory`, {
    method: 'POST',
    body: JSON.stringify({
      characterId: state.selectedCharacterId,
      type: 'fact',
...
async function editMemoryItem(memoryId) {
  if (!memoryId || !state.memoryBundle) return;
  const items = [
    ...(state.memoryBundle.pinned || []),
    ...(state.memoryBundle.auto || []),
    ...(state.memoryBundle.canon || []),
    ...(state.memoryBundle.conflicts || []),
    ...(state.memoryBundle.allItems || []),
  ];
  const current = items.find((item) => item.id === memoryId);
  if (!current) return;
  const text = prompt('Edit memory:', current.text || '');
  if (!text || !text.trim() || text.trim() === current.text) return;
  await api(`./api/memory/${encodeURIComponent(memoryId)}`, {
    method: 'PATCH',
...
function getMemoryTypeLabel(type = '') {
  const map = { fact: 'Fact', preference: 'Preference', relationship: 'Relationship', scenario: 'Scenario', canon: 'Canon', promise: 'Promise', emotion: 'Emotion', conflict: 'Conflict' };
  return map[String(type || '').toLowerCase()] || 'Memory';
}
...
function renderMemoryPanel() {
  ...
  if (!bundle) {
    const emptyMessage = state.memoryError || 'Pick a character and conversation to build memory.';
    if (memoryEls.relationship) memoryEls.relationship.textContent = emptyMessage;
    if (memoryEls.emotions) memoryEls.emotions.textContent = state.memoryError ? 'Load failed' : '—';
    if (memoryEls.scenario) memoryEls.scenario.textContent = state.memoryError || '—';
  ...
  if (memoryEls.status) {
    const counts = [
      `${(bundle.pinned || []).length} pinned`,
      `${(bundle.auto || []).length} auto`,
      `${(bundle.canon || []).length} canon`,
      `${(bundle.conflicts || []).length} conflicts`,
    ];
    memoryEls.status.textContent = `${counts.join(' · ')}${state.selectedConversationId ? ` · chat ${String(state.selectedConversationId).slice(-6)}` : ''}`;
  }
  if (memoryEls.pinned) memoryEls.pinned.innerHTML = renderMemoryList(bundle.pinned || [], { allowEdit: true, expanded: true, empty: 'No pinned memory yet.' });
  ...
  if (memoryEls.conflicts) memoryEls.conflicts.innerHTML = renderMemoryList(bundle.conflicts || [], { expanded: true, empty: 'No contradictions detected.' });

  if (memoryEls.addPinned) memoryEls.addPinned.onclick = () => { saveManualPinnedMemory().catch(() => null); };
  if (memoryEls.refresh) memoryEls.refresh.onclick = () => { loadMemoryBundle().catch(() => null); };
  memoryEls.pinned?.querySelectorAll('[data-memory-edit]').forEach((node) => node.addEventListener('click', async () => {
    await editMemoryItem(node.dataset.memoryEdit);
  }));
  ...
  memoryEls.auto?.querySelectorAll('[data-memory-promote]').forEach((node) => node.addEventListener('click', async () => {
    ...
    await loadMemoryBundle();
    if (el.appShell.classList.contains('memory-panel-open')) {
      setTimeout(() => { loadMemoryBundle().catch(() => null); }, 120);
    }
  }));
...
function setAuthMode(authenticated) {
  ...
  if (el.loginForm) {
    el.loginForm.setAttribute('autocomplete', 'on');
    if (authenticated) {
...
function applySettings(settings = getUiSettings()) {
  ...
  if (el.settingAutoAppendMemory) el.settingAutoAppendMemory.value = (settings.autoAppendAutoMemoryToPinned === false || settings.autoAppendAutoMemoryToPinned === 'off') ? 'off' : 'on';
...
function updateHeader() {
  ...
  const assistantBadgeLabel = isAssistantCharacter(character) ? getAssistantBadgeLabel() : '';
  const nextHeaderSignature = character ? `${character.id || title}:${character.imageUrl || ''}:${title}:${assistantBadgeLabel}` : title;
  ...
  const badgeHost = document.getElementById('current-character-badges');
  if (badgeHost) {
    badgeHost.innerHTML = isAssistantCharacter(character)
      ? `<span class="type-badge openclaw-badge">${escapeHtml(assistantBadgeLabel)}</span><span class="type-badge assistant-badge">Assistant</span>`
      : '';
  }
...
function syncRecentConversationsFromCurrentList() {
  ...
  const current = getCurrentConversationList().map((item) => ({
    ...
    bucketKey,
    characterType: item.characterType || getCharacterType(currentCharacter),
    assistantMeta: item.assistantMeta || getAssistantMetaFromCharacter(currentCharacter),
...
function renderRecentChats() {
  ...
  for (const [index, item] of recent.entries()) {
    ...
    const isAssistant = String(item.characterType || character.type || 'character') === 'assistant';
    const assistantBadge = isAssistant ? `<span class="type-badge openclaw-badge recent-openclaw-badge">${escapeHtml(item.assistantMeta?.badgeLabel || character.assistantConfig?.badgeLabel || 'OpenClaw')}</span>` : '';
    card.innerHTML = `
      ...
      <div class="recent-card-content">
        ...
        <div class="recent-card-meta">${item.name || 'Resume chat'} ${assistantBadge}</div>
...
function renderPersonas() {
  ...
  if (el.editPersonaBtn) el.editPersonaBtn.disabled = !getSelectedPersona();
}

function openPersonaEditDialog(mode = 'edit') {
  const persona = mode === 'edit' ? getSelectedPersona() : null;
  if (mode === 'edit' && !persona) return;
  el.personaEditTitle.textContent = mode === 'create' ? 'Create persona' : 'Edit persona';
  el.personaEditSave.textContent = mode === 'create' ? 'Create persona' : 'Save persona';
  el.personaEditForm.dataset.mode = mode;
  el.personaEditName.value = persona?.name || '';
  el.personaEditDescription.value = persona?.description || '';
  el.personaEditDefault.checked = mode === 'create' ? !state.bootstrap?.personas?.some((item) => item.isDefault) : !!persona?.isDefault;
  el.personaEditDialog.showModal();
}

async function savePersonaFromDialog() {
  const mode = el.personaEditForm.dataset.mode || 'edit';
  const persona = mode === 'edit' ? getSelectedPersona() : null;
  el.personaEditSave.disabled = true;
  el.personaEditSave.textContent = mode === 'create' ? 'Creating…' : 'Saving…';
  try {
    const payload = {
      name: el.personaEditName.value.trim(),
      description: el.personaEditDescription.value.trim(),
      isDefault: el.personaEditDefault.checked,
    };
    if (mode === 'create') {
      ...
      if (!persona) return;
      await api(`./api/personas/${encodeURIComponent(persona.id)}`, {
        method: 'PUT',
        body: JSON.stringify(payload),
      });
    }
    ...
    if (mode === 'create') {
      const created = (state.bootstrap?.personas || []).find((item) => item.name === payload.name);
      if (created) state.selectedPersonaId = created.id;
    }
    renderPersonas();
    updateHeader();
    rerenderChatIfMounted({ preserveScroll: true });
    el.personaEditDialog.close();
  } finally {
    el.personaEditSave.disabled = false;
    el.personaEditSave.textContent = mode === 'create' ? 'Create persona' : 'Save persona';
  }
}
...
async function generateAssistantTurn(messages, { conversationId, model, temperature, stream = true, onDelta = null, onDone = null } = {}) {
  const assistantMode = isAssistantCharacter();
  const response = await fetch('./api/chat', {
    ...
    body: JSON.stringify({
      ...
      personaId: state.selectedPersonaId,
      conversationId,
      model: assistantMode ? undefined : (model || getCharacterModel()),
      temperature: assistantMode ? undefined : (typeof temperature === 'number' ? temperature : getCharacterTemperature()),
      messages,
      stream: assistantMode ? false : stream,
    }),
  });

  const json = !response.ok || !stream || assistantMode || !response.body
  ...
  if (!assistantMode && stream && response.body) {
    let streamedText = '';
    let streamError = '';
  ...
  if (typeof onDone === 'function') onDone(content);
  return content;
}
...
function renderMessage(message, options = {}) {
  ...
  const showTts = message.role === 'assistant' && !isAssistantCharacter(character) && !!text.trim() && !!getCharacterFishReferenceId();
  ...
  row.innerHTML = `
    ...
    <div class="message-wrap ${showTts ? 'has-tts' : ''} ${actionAvailable ? 'message-wrap-actionable' : ''}">
      ...
      ` : ''}
      ${images.length ? `<div class="message-images">${images.map((src) => `<button class="message-image-button" type="button" data-lightbox-src="${escapeHtml(src)}" aria-label="Open image preview"><img src="${escapeHtml(src)}" alt="attachment" /></button>`).join('')}</div>` : ''}
    </div>
    ${message.role === 'user' && imageUrl ? `<img class="avatar-bubble" src="${imageUrl}" alt="${name}" />` : ''}
  `;
...
function updateMobileKeyboardState() {
  ...
  const focusInComposer = active === el.input;
  const keyboardLikelyOpen = Boolean(vv && isMobile && focusInComposer && (window.innerHeight - vv.height) > 120);
...
function buildImageStylePrompt(style = 'default') {
  const map = {
    default: '',
    anime: 'anime style, detailed anime illustration, expressive eyes, clean lineart',
    realistic: 'photorealistic, realistic skin, natural lighting, highly detailed',
    cinematic: 'cinematic lighting, dramatic composition, depth of field, film still',
    soft: 'soft portrait lighting, dreamy atmosphere, gentle colors, elegant portrait',
    nsfw: 'sensual pose, seductive atmosphere, detailed body, mature content',
  };
  return map[String(style || 'default')] || '';
}

function getInlineReferenceCacheKey(characterId = state.selectedCharacterId, personaId = state.selectedPersonaId) {
  return `aichat:inline-image-refs:${characterId || 'none'}:${personaId || 'none'}`;
}

function loadInlineReferenceCache() {
  const cacheKey = getInlineReferenceCacheKey();
  state.imageGeneration.referenceCacheKey = cacheKey;
  try {
    const refs = JSON.parse(localStorage.getItem(cacheKey) || '[]');
    state.imageGeneration.referenceImages = Array.isArray(refs)
      ? refs.map((item) => ({ name: item.name || '', type: item.type || 'image/png', url: item.url || item.dataUrl || '' })).filter((item) => item.url)
      : [];
  } catch {
    state.imageGeneration.referenceImages = [];
  }
}

function saveInlineReferenceCache() {
  const cacheKey = state.imageGeneration.referenceCacheKey || getInlineReferenceCacheKey();
  state.imageGeneration.referenceCacheKey = cacheKey;
  try {
    const refs = (state.imageGeneration.referenceImages || []).map((item) => ({ name: item.name || '', type: item.type || 'image/png', url: item.url || '' })).filter((item) => item.url);
    localStorage.setItem(cacheKey, JSON.stringify(refs));
  } catch {
    if (el.imageGenerateInlineStatus) el.imageGenerateInlineStatus.textContent = 'References kept for this session, but browser storage is full.';
  }
}

function renderInlineReferenceList() {
  if (!el.imageGenerateInlineReferenceList) return;
  const refs = state.imageGeneration.referenceImages || [];
  if (!refs.length) {
    el.imageGenerateInlineReferenceList.innerHTML = '<div class="inline-reference-empty">No uploaded references.</div>';
    return;
  }
  el.imageGenerateInlineReferenceList.innerHTML = refs.map((item, index) => {
    const safeName = escapeHtml(item.name || `reference-${index + 1}`);
    const safeUrl = escapeHtml(item.url || '');
    return `
      <div class="inline-reference-card">
        <img class="inline-reference-thumb" src="${safeUrl}" alt="${safeName}" />
        <button class="inline-reference-remove" type="button" data-remove-reference-index="${index}" aria-label="Remove ${safeName}">×</button>
        <div class="inline-reference-meta">${safeName}</div>
      </div>
    `;
  }).join('');
}

function openImageGenerateDialog(mode = 'default') {
  const character = getSelectedCharacter();
  if (!character) return;
  if (!el.imageGeneratePrompt.value.trim() || mode === 'default') {
    el.imageGeneratePrompt.value = `${character.name}, ${character.description || character.summary || 'high quality portrait'}`;
  }
  el.imageGenerateStatus.textContent = '';
  el.imageGenerateSubmit.disabled = false;
  el.imageGenerateSubmit.textContent = 'Generate';
  if (mode === 'variation' && state.imageGeneration.lastRequest?.style) el.imageGenerateStyle.value = state.imageGeneration.lastRequest.style;
  if (mode === 'variation' && state.imageGeneration.lastRequest?.aspectRatio) el.imageGenerateAspect.value = state.imageGeneration.lastRequest.aspectRatio;
  el.imageGenerateDialog.classList.remove('hidden');
  el.imageGenerateDialog.classList.add('dialog-force-open');
  el.imageGenerateDialog.setAttribute('aria-hidden', 'false');
  document.body.classList.add('modal-open');
  setTimeout(() => el.imageGeneratePrompt?.focus(), 40);
}

async function appendGeneratedImageToChat({ imageUrl, prompt, styleLabel = '', conversationId = state.selectedConversationId } = {}) {
  if (!conversationId) {
    await ensureConversation();
    conversationId = state.selectedConversationId;
  }
  const sameConversationSelected = conversationId && state.selectedConversationId === conversationId;
  if (sameConversationSelected) {
    const conversation = await fetchConversation(conversationId);
    const caption = styleLabel ? `Generated image (${styleLabel}): ${prompt}` : `Generated image: ${prompt}`;
    const nextMessages = [...(conversation?.messages || []), { role: 'assistant', content: caption, images: [{ url: imageUrl, name: 'generated-image', type: 'image/png' }] }];
    await updateConversation(conversationId, { messages: nextMessages });
  }
  delete state.imageGeneration.pendingByConversation[conversationId];
  if (sameConversationSelected) await hydrateSelectedWorkspace({ keepSelection: true, skipCharacterList: true });
  else await refreshBootstrapPreserveSelection().catch(() => null);
}

async function createImageGenerationPlaceholder({ prompt, styleLabel = '' } = {}) {
  await ensureConversation();
  const conversationId = state.selectedConversationId;
  const conversation = await fetchConversation(conversationId);
  const placeholderId = `img_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
  const caption = styleLabel ? `Generating image (${styleLabel})…
${prompt}` : `Generating image…
${prompt}`;
  const messages = [...(conversation?.messages || []), { role: 'assistant', content: caption, pendingImageId: placeholderId }];
  await updateConversation(conversationId, { messages });
  await hydrateSelectedWorkspace({ keepSelection: true, skipCharacterList: true });
  return { conversationId, placeholderId };
}

async function replaceImageGenerationPlaceholder({ conversationId, placeholderId, imageUrl, prompt, styleLabel = '' } = {}) {
  const sameConversationSelected = conversationId && state.selectedConversationId === conversationId;
  const conversation = await fetchConversation(conversationId);
  const caption = styleLabel ? `Generated image (${styleLabel}): ${prompt}` : `Generated image: ${prompt}`;
  let replaced = false;
  const messages = (conversation?.messages || []).map((message) => {
    if (!replaced && message?.pendingImageId === placeholderId) {
      replaced = true;
      return { role: 'assistant', content: caption, images: [{ url: imageUrl, name: 'generated-image', type: 'image/png' }] };
    }
    return message;
  });
  if (!replaced) messages.push({ role: 'assistant', content: caption, images: [{ url: imageUrl, name: 'generated-image', type: 'image/png' }] });
  await updateConversation(conversationId, { messages });
  delete state.imageGeneration.pendingByConversation[conversationId];
  if (sameConversationSelected) {
    state.renderedConversationId = null;
  ...
  await refreshBootstrapPreserveSelection().catch(() => null);
}

async function failImageGenerationPlaceholder({ conversationId, placeholderId, error } = {}) {
  if (!conversationId || !placeholderId) return;
  const conversation = await fetchConversation(conversationId).catch(() => null);
  if (!conversation) return;
  let replaced = false;
  const message = String(error?.message || error || 'Image generation failed');
  const messages = (conversation.messages || []).map((item) => {
    if (!replaced && item?.pendingImageId === placeholderId) {
      replaced = true;
      return { role: 'assistant', content: `Image generation failed: ${message}` };
    }
    return item;
  });
  if (replaced) await updateConversation(conversationId, { messages });
...
function conversationHasPendingImages(conversation) {
  return Array.isArray(conversation?.messages) && conversation.messages.some((item) => item?.pendingImageId);
}

function startImageGenerationPoller() {
  if (state.imageGeneration.pollTimer) return;
  state.imageGeneration.pollTimer = setInterval(async () => {
    if (document.hidden || !state.selectedConversationId) return;
    try {
      const conversation = await fetchConversation(state.selectedConversationId);
      if (conversationHasPendingImages(conversation)) {
        state.renderedConversationId = null;
        state.renderedChatSignatures = [];
        await renderChat();
        return;
      }
      clearInterval(state.imageGeneration.pollTimer);
      state.imageGeneration.pollTimer = null;
      state.renderedConversationId = null;
      state.renderedChatSignatures = [];
      await hydrateSelectedWorkspace({ keepSelection: true, skipCharacterList: true });
    } catch {}
...
async function submitImageGenerateDialog(mode = 'default') {
  if (!state.selectedCharacterId) return;
  const prompt = el.imageGeneratePrompt.value.trim();
  if (!prompt) throw new Error('Prompt is required');
  const style = el.imageGenerateStyle?.value || 'default';
  const aspectRatio = el.imageGenerateAspect?.value || '4:5';
  const stylePrompt = buildImageStylePrompt(style);
  const finalPrompt = [prompt, stylePrompt].filter(Boolean).join(', ');
  const requestBody = {
    prompt: finalPrompt,
    originalPrompt: prompt,
    negativePrompt: el.imageGenerateNegative.value.trim(),
    style,
    aspectRatio,
    strength: mode === 'variation' ? 0.6 : 0.45,
    guidanceScale: 7,
    variationOf: mode === 'variation' ? (state.imageGeneration.lastImageUrl || '') : '',
  };
  state.imageGeneration.lastRequest = requestBody;
  el.imageGenerateSubmit.disabled = true;
  if (el.imageGenerateVariation) el.imageGenerateVariation.disabled = true;
  if (el.imageGenerateRetry) el.imageGenerateRetry.disabled = true;
  el.imageGenerateSubmit.textContent = mode === 'variation' ? 'Varying…' : 'Generating…';
  el.imageGenerateStatus.textContent = 'Generating image with OpenAI Images 2.0…';
  try {
    const json = await api(`./api/characters/${encodeURIComponent(state.selectedCharacterId)}/generate-image`, {
      method: 'POST',
      body: JSON.stringify(requestBody),
    });
    const imageUrl = String(json.imageUrl || json.remoteUrl || '').trim();
    if (!imageUrl) throw new Error('No image returned');
    state.imageGeneration.lastImageUrl = imageUrl;
    el.imageGenerateStatus.textContent = 'Image generated. Added to chat.';
    await appendGeneratedImageToChat({ imageUrl, prompt, styleLabel: style === 'default' ? '' : style });
    el.imageGenerateDialog.classList.remove('dialog-force-open');
    el.imageGenerateDialog.classList.add('hidden');
    el.imageGenerateDialog.setAttribute('aria-hidden', 'true');
    document.body.classList.remove('modal-open');
  } finally {
    el.imageGenerateSubmit.disabled = false;
    if (el.imageGenerateVariation) el.imageGenerateVariation.disabled = false;
    if (el.imageGenerateRetry) el.imageGenerateRetry.disabled = false;
    el.imageGenerateSubmit.textContent = 'Generate';
  }
}

function updateAssistantConfigVisibility() {
  const isAssistant = (el.createCharacterType?.value || 'character') === 'assistant';
  el.assistantConfigFields?.classList.toggle('hidden', !isAssistant);
}
...
function openCreateCharacterDialog(prefill = {}, mode = 'create', editingCharacterId = null) {
  ...
  el.createCharacterType.value = prefill.type === 'assistant' ? 'assistant' : 'character';
  ...
  el.assistantOpenclawAgent.value = prefill.assistantConfig?.openclawAgentId || '';
  el.assistantRuntime.value = prefill.assistantConfig?.runtime || 'subagent';
  el.assistantPermissionTier.value = prefill.assistantConfig?.permissionTier || 'chat';
  el.assistantBadgeLabel.value = prefill.assistantConfig?.badgeLabel || 'OpenClaw';
  el.assistantThinking.value = prefill.assistantConfig?.thinking || 'low';
  el.assistantPersonalityPrompt.value = prefill.assistantConfig?.personalityPrompt || '';
  el.assistantStyleNotes.value = prefill.assistantConfig?.styleNotes || '';
  el.createCharacterAvatar.required = mode !== 'edit' || !prefill.imageUrl;
  updateCharacterAvatarPreview(prefill.imageUrl || '');
  updateAssistantConfigVisibility();
...
async function createCharacterFromDialog() {
  ...
  const type = el.createCharacterType?.value === 'assistant' ? 'assistant' : 'character';
  const payload = {
    ...
    assistantConfig: type === 'assistant' ? {
      enabled: true,
      openclawAgentId: el.assistantOpenclawAgent.value.trim(),
      runtime: el.assistantRuntime.value || 'subagent',
      permissionTier: el.assistantPermissionTier.value || 'chat',
      badgeLabel: el.assistantBadgeLabel.value.trim() || 'OpenClaw',
      thinking: el.assistantThinking.value || 'low',
      personalityPrompt: el.assistantPersonalityPrompt.value.trim(),
      styleNotes: el.assistantStyleNotes.value.trim(),
    } : null,
  };
...
function renderCharacterPanel() {
    ...
    <section class="info-card">
      ...
      <label class="field">
        ...
        <select id="character-tts-playback-mode">
          <option value="stream" ${getCharacterTtsPlaybackMode() === 'stream' ? 'selected' : ''}>Full-quality playback</option>
          <option value="full" ${getCharacterTtsPlaybackMode() === 'full' ? 'selected' : ''}>Wait for full audio</option>
        </select>
      </label>
      <div class="meta-line">Playback now uses one cleaned full-message request so first play matches replays.</div>
      <label class="field checkbox-field">
        <span>Read asterisk narration</span>
        <input id="character-tts-read-narration" type="checkbox" ${getCharacterTtsReadNarration() ? 'checked' : ''} />
      </label>
      <div class="meta-line">Off = dialogue only. On = also reads text inside <code>*asterisks*</code>.</div>
  ...
  el.characterPanelContent.querySelector('#character-tts-read-narration')?.addEventListener('change', (e) => {
    state.selectedCharacterSettings.ttsReadNarration = e.target.checked === true;
    state.tts.objectUrls = {};
    saveCharacterSettingsDebounced({ ttsReadNarration: state.selectedCharacterSettings.ttsReadNarration });
  });
...
async function refreshBootstrapPreserveSelection() {
  const prevCharacterId = state.selectedCharacterId;
  const prevPersonaId = state.selectedPersonaId;
  const prevConversationId = state.selectedConversationId;
  ...
  if (prevCharacterId) state.selectedCharacterId = prevCharacterId;
  if (prevPersonaId !== undefined) state.selectedPersonaId = prevPersonaId;
  if (prevConversationId) state.selectedConversationId = prevConversationId;
...
async function sendMessage() {
  if (state.isStreaming || state.sendInFlight) return;
  closeMessageActions();
  const selection = getCurrentSelectionBucket();
  ...
  syncMobileViewport();
  await ensureConversation();
  if (!isSameSelectionBucket(selection) || !state.selectedConversationId) {
  ...
  const conversationId = state.selectedConversationId;
  const conversation = await fetchConversation(conversationId);
  if (!isSameSelectionBucket(selection) || conversationId !== state.selectedConversationId) {
    state.sendInFlight = false;
    updateComposerState();
    return;
  }
  ...
  try {
    ...
    if (streamedText && getCharacterAutoplayVoiceEnabled() && characterHasVoice()) {
      ...
      playMessageTts(assistantMessage, assistantKey).catch((error) => {
        console.error('Autoplay TTS failed:', error);
      });
    }
...
function wireEvents() {
  ...
  // Mobile browsers can blur the textarea on the first tap before the click fires,
  // which made Send require two taps after the zoom-reset workaround. Capture the
  // pointer while the textarea is focused, keep focus stable for this tap, and send.
  el.send.addEventListener('pointerdown', (event) => {
    if (!isMobileViewport() || document.activeElement !== el.input) return;
    event.preventDefault();
    sendMessage();
  });
  ...
  el.chatView.addEventListener('click', async (event) => {
    ...
    if (imageButton) {
      event.preventDefault();
      event.stopPropagation();
      openImageLightbox(imageButton.getAttribute('data-lightbox-src') || imageButton.querySelector('img')?.src || '');
      return;
    }
  ...
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && !el.imageLightbox?.classList.contains('hidden')) closeImageLightbox();
  });
  ...
  const resetMobileComposerViewport = () => {
    document.body.classList.remove('keyboard-open');
    syncMobileViewport();
  };
  el.input.addEventListener('blur', () => {
    resetMobileComposerViewport();
  });

  el.newPersonaBtn?.addEventListener('click', () => openPersonaEditDialog('create'));
  el.editPersonaBtn?.addEventListener('click', () => openPersonaEditDialog());
  el.closePersonaEdit?.addEventListener('click', () => el.personaEditDialog.close());
  el.personaEditCancel?.addEventListener('click', () => el.personaEditDialog.close());
  el.personaEditForm?.addEventListener('submit', async (event) => {
    event.preventDefault();
    await savePersonaFromDialog();
  });
  ...
  const openInlineImagePanel = () => {
    const character = getSelectedCharacter();
    if (!character) {
      alert('Pick a character first.');
      return;
    }
    if (!el.imageGenerateInlinePrompt.value.trim()) {
      el.imageGenerateInlinePrompt.value = `${character.name}, ${character.description || character.summary || 'high quality portrait'}`;
    }
    loadInlineReferenceCache();
    if (el.imageGenerateInlineUseAvatar) el.imageGenerateInlineUseAvatar.checked = getUiSettings().imageUseAvatarReferenceDefault !== false;
    renderInlineReferenceList();
    el.imageGenerateInlineStatus.textContent = state.imageGeneration.referenceImages.length
      ? `Avatar reference: ${character.name}. ${state.imageGeneration.referenceImages.length} cached uploaded reference image(s) ready.`
      : `Avatar reference: ${character.name}. Uploads optional.`;
    el.imageGenerateInlinePanel.classList.remove('hidden');
    el.imageGenerateInlinePanel.setAttribute('aria-hidden', 'false');
    setTimeout(() => el.imageGenerateInlinePrompt?.focus(), 40);
  };
  const closeInlineImagePanel = () => {
    el.imageGenerateInlinePanel.classList.add('hidden');
    el.imageGenerateInlinePanel.setAttribute('aria-hidden', 'true');
  };
  el.imageGenerateInlineUseAvatar?.addEventListener('change', () => {
    saveAppUiSettingsDebounced({ imageUseAvatarReferenceDefault: !!el.imageGenerateInlineUseAvatar.checked });
    if (el.imageGenerateInlineUseAvatar.checked) {
      el.imageGenerateInlineStatus.textContent = `Avatar reference: ${getSelectedCharacter()?.name || 'character'}. Uploads optional.`;
      return;
    }
    el.imageGenerateInlineStatus.textContent = state.imageGeneration.referenceImages.length
      ? `Using ${state.imageGeneration.referenceImages.length} uploaded reference image(s).`
      : 'Avatar disabled. Upload at least one reference image.';
  });
  const submitInlineImagePanel = async (mode = 'default') => {
    if (!state.selectedCharacterId) return;
    const prompt = el.imageGenerateInlinePrompt.value.trim();
    if (!prompt) throw new Error('Prompt is required');
    const useAvatarReference = Boolean(el.imageGenerateInlineUseAvatar?.checked);
    const referenceImages = state.imageGeneration.referenceImages || [];
    if (!useAvatarReference && !referenceImages.length) throw new Error('Enable avatar reference or upload at least one reference image');
    const style = el.imageGenerateInlineStyle.value || 'default';
    const styleLabel = style === 'default' ? '' : style;
    const aspectRatio = el.imageGenerateInlineAspect.value || '4:5';
    const imageModel = el.imageGenerateInlineModel?.value || 'openai/gpt-image-2';
    const finalPrompt = [prompt, buildImageStylePrompt(style)].filter(Boolean).join(', ');
    const requestBody = {
      prompt: finalPrompt,
      originalPrompt: prompt,
      negativePrompt: el.imageGenerateInlineNegative.value.trim(),
      style,
      aspectRatio,
      imageModel,
      variationOf: mode === 'variation' ? (state.imageGeneration.lastImageUrl || '') : '',
      useAvatarReference,
      referenceImages: referenceImages.map((item) => ({ name: item.name || '', type: item.type || 'image/png', dataUrl: item.url })),
    };
    state.imageGeneration.lastRequest = requestBody;
    el.imageGenerateInlineSubmit.disabled = true;
    el.imageGenerateInlineStatus.textContent = 'Queueing image generation…';
    closeInlineImagePanel();
    const placeholder = await createImageGenerationPlaceholder({ prompt, styleLabel });
    try {
      const json = await api(`./api/characters/${encodeURIComponent(state.selectedCharacterId)}/generate-image`, {
        method: 'POST',
        body: JSON.stringify({
          ...
          personaId: state.selectedPersonaId || '',
          pendingImageId: placeholder.placeholderId,
        }),
      });
      if (!json?.ok) throw new Error('Image generation did not start');
      startImageGenerationPoller();
      el.imageGenerateInlineStatus.textContent = 'Image generation started.';
    } catch (error) {
      await failImageGenerationPlaceholder({ ...placeholder, error });
      throw error;
    } finally {
      el.imageGenerateInlineSubmit.disabled = false;
    }
  };
  el.imageGenerateInlineBtn?.addEventListener('click', openInlineImagePanel);
  el.closeImageGenerateInline?.addEventListener('click', closeInlineImagePanel);
  el.imageGenerateInlineReferenceInput?.addEventListener('change', async (event) => {
    const files = Array.from(event.target?.files || []);
    const nextRefs = files.length ? await filesToDataUrls(files) : [];
    state.imageGeneration.referenceImages = [...(state.imageGeneration.referenceImages || []), ...nextRefs];
    saveInlineReferenceCache();
    if (el.imageGenerateInlineReferenceInput) el.imageGenerateInlineReferenceInput.value = '';
    renderInlineReferenceList();
    if (!el.imageGenerateInlineUseAvatar?.checked && state.imageGeneration.referenceImages.length) {
      el.imageGenerateInlineStatus.textContent = `Using ${state.imageGeneration.referenceImages.length} uploaded reference image(s).`;
    }
  });
  el.imageGenerateInlineReferenceList?.addEventListener('click', (event) => {
    const button = event.target?.closest?.('[data-remove-reference-index]');
    if (!button) return;
    const index = Number(button.getAttribute('data-remove-reference-index'));
    if (!Number.isFinite(index) || index < 0) return;
    state.imageGeneration.referenceImages = (state.imageGeneration.referenceImages || []).filter((_, itemIndex) => itemIndex !== index);
    saveInlineReferenceCache();
    renderInlineReferenceList();
    if (!state.imageGeneration.referenceImages.length) {
      el.imageGenerateInlineStatus.textContent = el.imageGenerateInlineUseAvatar?.checked
        ? `Avatar reference: ${getSelectedCharacter()?.name || 'character'}. Uploads optional.`
        : 'No uploaded references.';
    }
  });
  el.imageGenerateInlinePromptFromChat?.addEventListener('click', async () => {
    if (!state.selectedCharacterId || !state.selectedConversationId) {
      el.imageGenerateInlineStatus.textContent = 'Open a conversation first.';
      return;
    }
    const button = el.imageGenerateInlinePromptFromChat;
    const originalText = button.textContent;
    button.disabled = true;
    button.textContent = 'Reading chat…';
    el.imageGenerateInlineStatus.textContent = 'Generating image prompt from current conversation…';
    try {
      const json = await api(`./api/characters/${encodeURIComponent(state.selectedCharacterId)}/generate-image-prompt`, {
        method: 'POST',
        body: JSON.stringify({
          conversationId: state.selectedConversationId,
          personaId: state.selectedPersonaId || '',
          useAvatarReference: Boolean(el.imageGenerateInlineUseAvatar?.checked),
        }),
      });
      const prompt = String(json.prompt || '').trim();
      if (!prompt) throw new Error('No prompt returned');
      el.imageGenerateInlinePrompt.value = prompt;
      el.imageGenerateInlinePrompt.dispatchEvent(new Event('input', { bubbles: true }));
      el.imageGenerateInlineStatus.textContent = `Prompt generated${json.provider ? ` via ${json.provider}` : ''}. Edit if needed, then Generate.`;
    } catch (error) {
      el.imageGenerateInlineStatus.textContent = error.message || 'Prompt generation failed';
    } finally {
      button.disabled = false;
      button.textContent = originalText;
    }
  });
  el.imageGenerateInlineSubmit?.addEventListener('click', async () => {
    try { await submitInlineImagePanel(); } catch (error) { el.imageGenerateInlineStatus.textContent = error.message || 'Image generation failed'; }
  });
  el.imageGenerateInlineVariation?.addEventListener('click', async () => {
    try { await submitInlineImagePanel('variation'); } catch (error) { el.imageGenerateInlineStatus.textContent = error.message || 'Variation failed'; }
  });
  el.imageGenerateInlineRetry?.addEventListener('click', async () => {
    if (!state.imageGeneration.lastRequest) {
      el.imageGenerateInlineStatus.textContent = 'No previous image request yet.';
      return;
    }
    el.imageGenerateInlinePrompt.value = state.imageGeneration.lastRequest.originalPrompt || state.imageGeneration.lastRequest.prompt || '';
    el.imageGenerateInlineNegative.value = state.imageGeneration.lastRequest.negativePrompt || '';
    el.imageGenerateInlineStyle.value = state.imageGeneration.lastRequest.style || 'default';
    el.imageGenerateInlineAspect.value = state.imageGeneration.lastRequest.aspectRatio || '4:5';
    if (el.imageGenerateInlineUseAvatar) el.imageGenerateInlineUseAvatar.checked = state.imageGeneration.lastRequest.useAvatarReference !== false;
    state.imageGeneration.referenceImages = Array.isArray(state.imageGeneration.lastRequest.referenceImages)
      ? state.imageGeneration.lastRequest.referenceImages.map((item) => ({ name: item.name || '', type: item.type || 'image/png', url: item.dataUrl || item.url || '' })).filter((item) => item.url)
      : [];
    saveInlineReferenceCache();
    renderInlineReferenceList();
    try { await submitInlineImagePanel('retry'); } catch (error) { el.imageGenerateInlineStatus.textContent = error.message || 'Retry failed'; }
  });
  ...
  const closeImageGenerateModal = () => {
    el.imageGenerateDialog?.classList.remove('dialog-force-open');
    el.imageGenerateDialog?.classList.add('hidden');
    el.imageGenerateDialog?.setAttribute('aria-hidden', 'true');
    document.body.classList.remove('modal-open');
  };
  el.closeImageGenerate?.addEventListener('click', closeImageGenerateModal);
  document.getElementById('image-generate-backdrop')?.addEventListener('click', closeImageGenerateModal);
  el.imageGenerateCancel?.addEventListener('click', closeImageGenerateModal);
  el.imageGenerateVariation?.addEventListener('click', async () => {
    try {
      await submitImageGenerateDialog('variation');
    } catch (error) {
      el.imageGenerateStatus.textContent = error.message || 'Variation failed';
    }
  });
  el.imageGenerateRetry?.addEventListener('click', async () => {
    if (!state.imageGeneration.lastRequest) {
      el.imageGenerateStatus.textContent = 'No previous image request yet.';
      return;
    }
    el.imageGeneratePrompt.value = state.imageGeneration.lastRequest.originalPrompt || state.imageGeneration.lastRequest.prompt || '';
    el.imageGenerateNegative.value = state.imageGeneration.lastRequest.negativePrompt || '';
    el.imageGenerateStyle.value = state.imageGeneration.lastRequest.style || 'default';
    el.imageGenerateAspect.value = state.imageGeneration.lastRequest.aspectRatio || '4:5';
    try {
      await submitImageGenerateDialog('retry');
    } catch (error) {
      el.imageGenerateStatus.textContent = error.message || 'Retry failed';
    }
  });
  el.imageGenerateForm?.addEventListener('submit', async (event) => {
    event.preventDefault();
    try {
      await submitImageGenerateDialog();
    } catch (error) {
      el.imageGenerateStatus.textContent = error.message || 'Image generation failed';
    }
  });
  ...
  el.createCharacterType?.addEventListener('change', updateAssistantConfigVisibility);
